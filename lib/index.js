...
function Oplog(conn, options) {
  if (!(this instanceof Oplog)) return new Oplog(conn, options);
...
Oplog.prototype.tail = function tail() {

  debug('Connected to oplog database');

  var oplog = this;

  oplog.conn.db(function database(err, db) {
    ...
    var time
      , since
      , query = {}
      , collection = db.collection(oplog.coll)
      , options = {
          tailable: true,
          awaitdata: true,
          timeout: false,
    ...
    collection
      ...
      .nextObject(function next(err, doc) {
        ...
        if (doc) {
          ...
          oplog.emit('stream', oplog.stream);
          oplog.bind();
        } else {
          oplog.stop();
        }
