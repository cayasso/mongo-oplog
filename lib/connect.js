...
var Emitter = require('events').EventEmitter
  ...
  , debug = require('debug')('mongo-oplog:connection');
...
Connection.prototype.connect = function connect(uri, options, fn) {
  var connection = this;
  MongoClient.connect(uri, options, function connect(err, adminDb) {
    if (err) {
      debug('error connecting %j', err);
      if (fn) fn(err);
      return;
    }
    var db = adminDb.db(options.database || 'local');
...
Connection.prototype.ready = function (fn) {
